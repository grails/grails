...
* _StatusUpdate_ - When the plug-in list is being refreshed
* _StatusError_ - When there was an error updating the plug-in list
...
The first column contains the plugin name, the second the version and the last the description. If you require more info about a plug-in you can use the "plugin-info":plugin-info.html command. If you wish to install a plug-in you can use the plug-in name and/or version in combination with the "install-plugin":install-plugin.html command.


Note: If you are behind a proxy you may want to consider using the "set-proxy":set-proxy.html command prior to running this command.