			...
			<g:if test="${session?.user}">
				<div style="margin-top:20px">
					<div style="float:right;">
						<a href="#"><g:message code="gtunes.profile"/></a> | <g:link controller="user" action="logout"><g:message code="gtunes.logout"/></g:link><br>						
					</div>
					<g:message code="gtunes.welcome"/> <span id="userFirstName">${session?.user?.firstName}</span>!<br><br>
					<g:message code="gtunes.purchased.songs" args="[session.user.purchasedSongs?.size() ?: 0]"/><br>
				</div>
			</g:if>
			<g:else>
			...
			<g:if test="${session.user}">
				<div id="navButtons">
					<ul>
						...
						<li><g:link controller="store" action="shop"><g:message code="gtunes.the.store"/></g:link></li>				
					</ul>
				</div>
			</g:if>
			<g:else>
