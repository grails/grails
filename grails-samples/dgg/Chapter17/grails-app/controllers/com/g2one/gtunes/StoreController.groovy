...
class StoreController {
	...
	def latest = {
		def newestAlbums = Album.list(offset:1,max:5, sort:"dateCreated", order:"asc")
		
		def feed = {
			title = "Newest Additions to gTunes"
			link = g.createLink(controller:"store", action:"latest", params:[format:request.format])
			description = "Track the newest additions to the gTunes music store"
			for(a in newestAlbums) {
				entry(a.title) {
					link = g.createLink(controller:"album", action:"show", id:a.id)
					g.render(template:"/album/album", model:[album:a, artist:a.artist])
				}
			}
		}
		...
		withFormat {
			rss {
				render(feedType:"rss", feed)
			}
			atom {
				render(feedType:"atom", feed)
			}
		}
	}
