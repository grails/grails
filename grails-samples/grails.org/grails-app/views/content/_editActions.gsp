...
<div id="editLinks" class="wikiLinks">
    ...
    <ul class="wikiActionMenu">
        ...
        <li>
            <a href="${href}" class="actionIcon"
                    onclick="new Ajax.Updater('${updateElement}', '${createLink(controller:'content',action:'saveWikiPage',id:content?.title, params:[update:updateElement])}', getAjaxOptions());
                    showCommentPost();
                    return false">
        ...
        <li>
            <g:remoteLink class="actionIcon" update="${updateElement}" controller="content" id="${content?.title}" params="[xhr:true,update:updateElement]" onLoad="showCommentPost()">
                <img src="${createLinkTo(dir: 'images/', 'icon-cancel.png')}" width="15" height="15" alt="Icon Cancel" class="inlineIcon" border="0"/>
                <span>Cancel</span>
            </g:remoteLink>
        </li>
        <li>
            <g:remoteLink class="actionIcon" action="infoWikiPage" id="${content?.title}" params="[update:updateElement]" update="${updateElement}" onLoad="showCommentPost()">
